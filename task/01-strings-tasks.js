...
function getRectangleString(width, height) {    
    ...
    let firstStr =`${elements[0]}${elements[1].repeat(width-2)}${elements[2]}\n` ;
    let middleStr=`${elements[3]}${elements[7].repeat(width-2)}${elements[3]}\n`.repeat(height-2);
    let lastStr=`${elements[4]}${elements[1].repeat(width-2)}${elements[5]}\n`;    
...
function encodeToRot13(str) {    
    ...
    let reverseAlph ='NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm!,.?; ';
    let answer ='';
    for (let i = 0; i < str.length; i++){
        for(let k = 0; k < alph.length; k++){
            if(str[i]==alph[k])
            {
                answer +=reverseAlph[k];
            }
        }
    }
  return answer;
